...
pub trait WidgetImplExt {
    ...
    fn parent_size_allocate(&self, widget: &Widget, allocation: &mut Allocation);
}
    ...
    T: WidgetImpl,
{
    ...
    let imp = instance.get_impl();
    let wrap: Widget = from_glib_borrow(ptr);
    let mut allocate: Allocation = from_glib_none(allocation);

    imp.size_allocate(&wrap, &mut allocate);
    *allocation = *allocate.to_glib_none().0;
}
