...
impl<O: IsA<IconView>> IconViewExt for O {
    fn convert_widget_to_bin_window_coords(&self, wx: i32, wy: i32) -> (i32, i32) {
        unsafe {
            ...
            gtk_sys::gtk_icon_view_convert_widget_to_bin_window_coords(self.as_ref().to_glib_none().0, wx, wy, bx.as_mut_ptr(), by.as_mut_ptr());
    ...
    fn get_dest_item_at_pos(&self, drag_x: i32, drag_y: i32) -> Option<(TreePath, IconViewDropPosition)> {
        unsafe {
            ...
            let ret = from_glib(gtk_sys::gtk_icon_view_get_dest_item_at_pos(self.as_ref().to_glib_none().0, drag_x, drag_y, &mut path, pos.as_mut_ptr()));
    ...
    fn get_drag_dest_item(&self) -> (TreePath, IconViewDropPosition) {
        unsafe {
            let mut path = ptr::null_mut();
            let mut pos = mem::MaybeUninit::uninit();
            gtk_sys::gtk_icon_view_get_drag_dest_item(self.as_ref().to_glib_none().0, &mut path, pos.as_mut_ptr());
            let pos = pos.assume_init();
    ...
    fn emit_activate_cursor_item(&self) -> bool {
        ...
        res.unwrap().get().expect("Return Value for `emit_activate_cursor_item`").unwrap()
    }
