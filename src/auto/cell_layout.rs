...
pub trait CellLayoutExt: 'static {
    ...
    fn set_cell_data_func<P: IsA<CellRenderer>>(&self, cell: &P, func: Option<Box_<dyn Fn(&CellLayout, &CellRenderer, &TreeModel, &TreeIter) + 'static>>);
}
...
impl<O: IsA<CellLayout>> CellLayoutExt for O {
    ...
    fn set_cell_data_func<P: IsA<CellRenderer>>(&self, cell: &P, func: Option<Box_<dyn Fn(&CellLayout, &CellRenderer, &TreeModel, &TreeIter) + 'static>>) {
        ...
        unsafe {
            gtk_sys::gtk_cell_layout_set_cell_data_func(self.as_ref().to_glib_none().0, cell.as_ref().to_glib_none().0, func, Box_::into_raw(super_callback0) as *mut _, destroy_call4);
        }
    }
}
