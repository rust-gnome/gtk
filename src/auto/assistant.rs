...
impl<O: IsA<Assistant>> AssistantExt for O {
    ...
    fn set_forward_page_func(&self, page_func: Option<Box_<dyn Fn(i32) -> i32 + 'static>>) {
        ...
        unsafe {
            gtk_sys::gtk_assistant_set_forward_page_func(self.as_ref().to_glib_none().0, page_func, Box_::into_raw(super_callback0) as *mut _, destroy_call3);
        }
    }
    ...
    fn get_property_use_header_bar(&self) -> i32 {
        unsafe {
            ...
            value.get().expect("Return Value for property `use-header-bar` getter").unwrap()
        }
    }
    ...
    fn get_child_complete<T: IsA<Widget>>(&self, item: &T) -> bool {
        unsafe {
            ...
            value.get().expect("Return Value for property `complete` getter").unwrap()
        }
    }
    ...
    fn get_child_has_padding<T: IsA<Widget>>(&self, item: &T) -> bool {
        unsafe {
            ...
            value.get().expect("Return Value for property `has-padding` getter").unwrap()
        }
    }
    ...
    fn get_child_page_type<T: IsA<Widget>>(&self, item: &T) -> AssistantPageType {
        unsafe {
            ...
            value.get().expect("Return Value for property `page-type` getter").unwrap()
        }
    }
