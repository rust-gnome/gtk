...
pub fn accelerator_parse(accelerator: &str) -> (u32, gdk::ModifierType) {
    ...
    unsafe {
        ...
        gtk_sys::gtk_accelerator_parse(accelerator.to_glib_none().0, accelerator_key.as_mut_ptr(), accelerator_mods.as_mut_ptr());
...
pub fn print_run_page_setup_dialog_async<P: IsA<Window>, Q: FnOnce(&PageSetup) + Send + Sync + 'static>(parent: Option<&P>, page_setup: Option<&PageSetup>, settings: &PrintSettings, done_cb: Q) {
    ...
    unsafe {
        gtk_sys::gtk_print_run_page_setup_dialog_async(parent.map(|p| p.as_ref()).to_glib_none().0, page_setup.to_glib_none().0, settings.to_glib_none().0, done_cb, Box_::into_raw(super_callback0) as *mut _);
    }
}
