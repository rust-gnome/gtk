...
impl<O: IsA<RecentChooser>> RecentChooserExt for O {
    ...
    fn get_uris(&self) -> Vec<GString> {
        unsafe {
            ...
            let ret = FromGlibContainer::from_glib_full_num(gtk_sys::gtk_recent_chooser_get_uris(self.as_ref().to_glib_none().0, length.as_mut_ptr()), length.assume_init() as usize);
            ret
        }
    }
    ...
    fn set_sort_func<P: Fn(&RecentInfo, &RecentInfo) -> i32 + 'static>(&self, sort_func: P) {
        ...
        unsafe {
            gtk_sys::gtk_recent_chooser_set_sort_func(self.as_ref().to_glib_none().0, sort_func, Box_::into_raw(super_callback0) as *mut _, destroy_call3);
        }
    }
